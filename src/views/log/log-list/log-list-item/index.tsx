...
import { isShowLog, isStickLog, deleteLogCont, makeLogTag } from '@/client/LogHelper';
      ...
      {isLogin &&
        <div className={styles.logOperateBox}>
          <Icon
            ...
            className={`${logItemData.tag.length !== 0 ? styles.active : ''} ${styles.logOperateIcon}`}
            title={'点击设置该日志的 tag'}
            type="book" />
          <Icon
      ...
      <div onClick={(e) => e.stopPropagation()}>
        ...
        >
          <Select
            ...
            value={tag}
            style={{ width: 200 }}
            onChange={(val: any) => { setTag(val) }}
          >
            {
              tagList.map((item: { tag_id: string; tag_name: string }) => {
                return (
                  <Option key={item.tag_id} value={item.tag_id}>{item.tag_name}</Option> 
                )
              })
            }
          </Select>
