...
const LogListItem: React.FC<PropsType> = (props) => {
  ...
  const [tag, setTag] = useState(blogItemData.tag.map((item: any) => item.tag_id));
        ...
        {isLogin && (
          <>
            <span className={styles.editType}>
              ({blogItemData.edittype === "richtext" ? "富文本文档" : "markdown"}
              )
            </span>
            <span className={styles.visits}>访问量：{blogItemData.visits}</span>
          </>
        )}
      </div>
